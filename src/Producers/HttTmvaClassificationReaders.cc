...
void AntiTtbarDiscriminatorTmvaReader::Init(spec_setting_type const& settings)
{
	...
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_pzetavis", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		return product.pZetaVis;
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_pzetamiss", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		return product.pZetaMissVis;
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_dphi", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		return std::abs(ROOT::Math::VectorUtil::DeltaPhi(product.m_flavourOrderedLeptons[0]->p4,
		                                                 product.m_flavourOrderedLeptons[1]->p4));
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_mvamet", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		return product.m_met.p4.Pt();
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_mtll", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		return Quantities::CalculateMt(product.m_flavourOrderedLeptons[0]->p4,
		                               product.m_flavourOrderedLeptons[1]->p4);
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_csv", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		float csv = -1.0;
	...
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva_d01", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
		return product.m_flavourOrderedLeptons[0]->track.getDxy(&(event.m_vertexSummary->pv));
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("emAntiTTbarMva", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
...
void TauPolarisationTmvaReader::Init(spec_setting_type const& settings)
{
	...
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("tauPolarisationTMVA", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
	...
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity("tauPolarisationSKLEARN", [](spec_event_type const& event, spec_product_type const& product, spec_setting_type const& settings)
	{
