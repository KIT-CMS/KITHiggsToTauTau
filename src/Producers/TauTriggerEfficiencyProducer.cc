		...
		for (auto t: settings.GetTauTriggerIDTypes())
		{
			for(auto weightNames: m_weightNames)
			{
				...
				for(size_t index = 0; index < weightNames.second.size(); index++)
				{
					...
					{
						product.m_weights[weightNames.second.at(index)+"_"+wp+"_"+t+"_"+std::to_string(weightNames.first+1)] = 1;
						product.m_weights[weightNames.second.at(index)+"Up_"+wp+"_"+t+"_"+std::to_string(weightNames.first+1)] = 1;
						product.m_weights[weightNames.second.at(index)+"Down_"+wp+"_"+t+"_"+std::to_string(weightNames.first+1)] = 1;
					}
				}
			}
		}
